...
live progress. The multi-stream cycle orchestration itself — loop over every
registered stream, always pull-then-push, worst-of outcome ranking, account-wide
failure isolation, per-stream `lastSyncAt` stamping — is `runSyncCycle` in
**`syncCycle.ts`** (issue #63); `src/store/appStore.ts`'s `drainSync` decides
*when* to connect/sync (manual-only, the "Sync now" button), owns the re-entrancy
guard, and is `runSyncCycle`'s sole caller, supplying `pullStream`/`drainStream` as
its injected deps — so it remains the only consumer of
`connect`/`disconnect`/`pullStream`/`drainStream` even though it no longer runs the
loop over them itself. `src/App.tsx` renders `ReconnectPill`.
...
- `class DriveError extends Error` — carries `status: number` and `reason?: string` (Drive's `error.errors[0].reason`, e.g. `storageQuotaExceeded`, `rateLimitExceeded`, when the body parsed and carried one). Getters: `isQuotaExceeded` (403 with reason `storageQuotaExceeded` — Drive is full; SPEC §8.4.5); `isAuth` (401, or 403 that isn't quota-exceeded or rate-limited — stop and reconnect); `isRetryable` (429, ≥500, or a 403 whose reason is `rateLimitExceeded`/`userRateLimitExceeded`/`dailyLimitExceeded` — back off and retry). Issue #88: Drive reports both a full-Drive condition and rate limiting as 403, which used to misclassify as auth and loop the reconnect pill forever on a full Drive (reconnecting can never fix that). Thrown by every helper on a non-OK response, with the message and reason extracted from the JSON error body when available.
...
- `type DrainOutcome = 'idle' | 'drained' | 'reconnect' | 'retry-later' | 'quota' | 'error'`.
- `interface DrainResult { outcome: DrainOutcome; uploaded: number; error?: string }` — `uploaded` counts events (not files); `error` is set for `'error'`, `'retry-later'`, and `'quota'` (issue #64: threaded through so Settings can show *why* Drive said retry-later, not just that it did).
...
- **Concurrent-drain-safe id assignment (issue #50).** `assignFileIds` persists newly-minted ids through `src/store/events#mergeFileIds`, an atomic read-current-row-then-merge inside one IndexedDB transaction, rather than blind-overwriting a possibly-stale in-memory row: if a concurrent drain (another tab, or a same-tab re-entry) already persisted an id for a name, that id wins and this call's freshly-minted one for the same name is simply discarded (Drive ids are free to mint and leave unused). This is defense-in-depth — the primary fix is the cross-tab `navigator.locks` lock around the whole cycle in `appStore.drainSync`, which should make two drains running at once unreachable in practice; this closes the gap anyway for whenever that lock is unavailable or two callers otherwise slip past it. Only the `fileIds` field is merged this way; other row fields (`status`, `attempts`, `phase`) are still written by ordinary `putSyncStatus` calls elsewhere in this module, which the lock is relied on to make safe.
...
- Failure classification (via `DriveError`): quota-exceeded (403 `storageQuotaExceeded`, SPEC §8.4.5) → batch rows re-queued, drain stops, outcome `'quota'` (issue #88 — distinct from auth: the token is fine, only Drive's storage is full, so a reconnect prompt could never fix it); auth (401, or 403 that isn't quota/rate-limit) → batch rows re-queued, drain stops, outcome `'reconnect'`; retryable (429/5xx, or a rate-limited 403) → batch rows re-queued, drain stops, outcome `'retry-later'` (the next "Sync now" retries them immediately, re-planning the batch from the persisted assignment); anything else → batch rows marked `'error'` and, below `MAX_ATTEMPTS_BEFORE_PARKED`, the drain stops (outcome `'error'`). In every failure path `attempts` is incremented and the message stored on the rows.
...
(eager: full offline availability, except audio when `keepAudioLocally` is off —
issue #53, see below), and imports them atomically via
...
- `interface PullResult { outcome: PullOutcome; pulled: number; error?: string }` — `pulled` counts events imported; `error` is set for `'error'` and `'retry-later'` (same threading as the drainer's `DrainResult`, issue #64).
...
- **Eager attachment download — except audio when `keepAudioLocally` is off (issue #53).** For each missing event, every referenced attachment blob not already local is fetched (segment members reference their attachments by their own per-event names), *unless* it is an audio attachment and `getStreamSettings(stream).keepAudioLocally` is `false` — read once per `pullStream` call — in which case it is left unfetched. The event still imports either way; without this a pull would silently re-inflate audio the push side had already pruned (`queue.ts#pruneAudio`), or another device's audio the setting says never to keep, defeating the setting the moment sync is bidirectional. One missing on Drive (pruned, or a §5.2 push race — the carrier commits last) is skipped and picked up on a later pull.
...
### src/drive/syncCycle.ts

Multi-stream sync cycle orchestration (issue #63), extracted out of
`store/appStore.ts`'s `drainSync` action: pull-then-push per registered
stream, worst-of outcome ranking, account-wide failure isolation, and
per-stream `lastSyncAt` stamping. This is Drive-protocol orchestration, not
UI state — both halves of the cycle it drives (`pullStream`/`drainStream`)
already live in `drive/`, so the loop moved here too, unit-testable without
touching zustand (`syncCycle.test.ts` drives it with fake `pull`/`drain`/
`setLastSyncAt` functions instead of mocking `drive/pull`/`drive/queue`).
`appStore.drainSync` now owns only token acquisition, the re-entrancy guard
(in-memory flag + cross-tab `navigator.locks`), the `cycle-start`/`cycle-done`
progress boundary (so `cycle-done` still fires from its `finally` if the
cycle throws), `set()` calls mirroring this module's result into store
state, and the post-cycle `refresh()`/`loadSettings()` — see
[store.md](store.md) for how it wires this module in.

Key exports:

- `interface StreamSyncResult { stream; outcome: DrainOutcome; uploaded; pulled: number; error? }` — one stream's slice of a cycle.
- `type SyncOutcome = DrainOutcome | 'busy'` and `interface SyncResult { outcome: SyncOutcome; uploaded; pulled: number; error?; perStream: StreamSyncResult[] }` — the aggregate cycle result; re-exported from `store/appStore.ts` so existing consumers (e.g. `settings/SettingsScreen.tsx`) don't need to know these types moved.
- `const OUTCOME_RANK: Record<DrainOutcome, number>` — the worst-of ordering (`idle < drained < retry-later < quota < reconnect < error`) used both per-stream (pull outcome vs. push outcome) and across streams (the aggregate).
- `interface SyncCycleDeps { pull; drain; setLastSyncAt; now: () => string; onProgress? }` — the effectful dependencies `runSyncCycle` is parameterized over; `now` is injected (rather than calling `toLocalIso(new Date())` internally) so tests get a deterministic stamp.
- `interface SyncCycleOutcome { result: SyncResult; reconnect: boolean; quotaExceeded: boolean }` — `runSyncCycle`'s return shape. `reconnect`/`quotaExceeded` are carried separately from `result.outcome` because that field is the *worst-of* outcome across every stream: a stream that errored before the account-wide abort can outrank (`OUTCOME_RANK`) the reconnect/quota that caused it, so the aggregate outcome alone can't tell the caller which account-wide condition (if any) actually happened.
- `runSyncCycle(token, streams, deps): Promise<SyncCycleOutcome>` — the loop itself: pull before push per stream (local appends land after everything the remote log already has); a `'reconnect'` or `'quota'` from any stream's pull/push is account-wide, so the remaining streams are skipped (marked with that same outcome) rather than burning more calls that would fail identically, while `'retry-later'`/`'error'` on one stream never blocks the others; each stream whose own outcome is clean (`'idle'`/`'drained'`) gets its own `setLastSyncAt` call, independent of the rest. Emits `stream-start`/`stream-done` (and forwards `pull-progress`/`upload-start`/`upload-progress` from `pull`/`drain`) through `deps.onProgress`; does **not** emit `cycle-start`/`cycle-done` itself — those stay the caller's responsibility (see above).
...
- **Pull before push.** The local IndexedDB is a replica of the Drive log; `runSyncCycle` (`syncCycle.ts`, called by `appStore.drainSync`) always runs `pullStream` then `drainStream` for each registered stream in turn (`allSyncStreams()` — see [store.md](store.md)), so local appends land after everything other devices committed. Identity is the event `id` — a seq collision across devices is expected and resolved by the fold's `seq → loggedAt → id` order (SPEC §3.3), never by the sync layer.
- **`keepAudioLocally` is honored on both sides of sync (issue #53).** `queue.ts#pruneAudio` drops local audio after a successful *upload* when the setting is off; `pull.ts#importPartition` skips *downloading* audio for the same reason, so a pull can never re-inflate audio the setting says to never keep (whether it's this device's own previously-pruned audio, or another device's history). Neither side ever deletes/skips a non-audio attachment for this setting. Reclaiming blobs the *fold* has hidden (a `revoke`d or `removeAttachments`-dropped attachment, regardless of kind) is a separate mechanism — `src/store/blobGc.ts` — since that is about visibility, not this per-kind local-storage preference.
...
- **Progress reporting is opt-in and coarse.** `onProgress` on `pullStream`/`drainStream` defaults to a no-op, so it never changes behavior for a caller that doesn't pass one (every existing test). Boundaries are deliberately no finer than one imported partition (pull) or one committed batch — a lone record or a whole segment (push) — never per file or per line; `runSyncCycle` (`syncCycle.ts`) owns the coarser per-stream boundaries (`stream-start`/`stream-done`) since it now runs that loop, while `appStore.drainSync` owns the outermost `cycle-start`/`cycle-done` pair around the call to it. The event shape (`SyncProgressEvent`) and everything that interprets it live in `src/store/syncProgress` — see [store.md](store.md).
