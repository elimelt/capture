...
- **Legacy rows keep the old probe and are never batched**: a row written by an older app version that already attempted an upload (`attempts > 0` or `phase === 'record-pending'` with no `fileIds`) may have files on Drive under names we hold no ids for — those rows keep the find-before-upload `findFile`, alone, so a retry never duplicates them.
...
- Failure classification (via `DriveError`): auth (401/403) → batch rows re-queued, drain stops, outcome `'reconnect'`; retryable (429/5xx) → batch rows re-queued, drain stops, outcome `'retry-later'` (the next "Sync now" retries them immediately, re-planning the batch from the persisted assignment); anything else → batch rows marked `'error'`, drain stops, outcome `'error'`. In every failure path `attempts` is incremented and the message stored on the rows.
