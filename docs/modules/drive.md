...
- Failure classification (via `DriveError`): auth (401/403) → row re-queued, drain stops, outcome `'reconnect'`; retryable (429/5xx) → row re-queued, drain stops, outcome `'retry-later'` (the next "Sync now" retries it immediately); anything else → row marked `'error'`, drain stops, outcome `'error'`. In every failure path `attempts` is incremented and the message stored on the row.
