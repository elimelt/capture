...
Pure data behind the tab bar, pulled out of `App.tsx` so it can be pinned by a hermetic test without importing the whole app shell: `interface NavTab { to; label; button?; assistant? }`, the `TABS` array (`Capture` `/`, `Today` `/day`, `Context` `/context` — button navigation, `Recall` `/chat` — `assistant: true`, `Settings` `/settings`), and `visibleTabs(tabs, assistantEnabled)`, a pure filter dropping assistant-gated tabs when disabled. `navTabs.test.ts` pins the route paths and current labels.
