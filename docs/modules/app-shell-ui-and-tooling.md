...
- **Routing**: `Routes` for `/` (CaptureScreen), `/day` and `/day/:date` (DayScreen), `/context` (ContextScreen), `/chat` (a private `ChatRoute`, guarded by `assistantEnabled` — otherwise `Navigate` to `/`), `/settings` (SettingsScreen), and a catch-all redirect to `/`. Route paths are frozen independent of tab labels (issue #84): the `/chat` path stays even though its tab now reads "Recall", because deep links, the SW precache manifest, and the `ChatScreen` chunk-name coupling below are all label-independent.
