...
`index.html` boots a pure-HTML/CSS splash screen, loads the Google Identity Services script (the only third-party runtime script), and sets a strict Content-Security-Policy plus iOS standalone-PWA meta tags. `src/main.tsx` registers the service worker, requests persistent storage, and renders `<App />` inside `StrictMode → ErrorBoundary → BrowserRouter`. `src/App.tsx` owns the route table (Capture `/`, Day `/day` and `/day/:date`, opt-in Chat `/chat`, Settings `/settings`), the fixed bottom tab bar, and app-level lifecycle effects: hydrating the Zustand store, fading out the splash, refreshing local state when the app returns to the foreground (Drive sync is manual-only, via "Sync now" in Settings), running background transcription/captioning whenever entries change, and showing a global error toast. `src/config.ts` holds the build-time constants: the public Google OAuth client ID, the OAuth scopes, and the `ENDPOINTS` block consumed by `assistant/`, `vision/`, and `transcribe/` (issue #69).
...
- **Routing**: `Routes` for `/` (CaptureScreen), `/day` and `/day/:date` (DayScreen), `/chat` (lazy `ChatScreen`, guarded by `assistantEnabled` — otherwise `Navigate` to `/`), `/settings` (SettingsScreen), and a catch-all redirect to `/`. `ChatScreen` is `lazy()`-loaded so users who never enable the assistant never download the AI SDK + markdown bundle. Route paths are frozen independent of tab labels (issue #84): the `/chat` path stays even though its tab now reads "Recall", because deep links, the SW precache manifest, and the `ChatScreen` chunk-name coupling below are all label-independent.
- **Tab bar**: a fixed, translucent, backdrop-blurred bottom `nav` on `layer.nav`, built from the `TABS` array (`src/navTabs.ts`); the "Recall" (`/chat`) tab is filtered out via `visibleTabs` unless `appSettings.assistantEnabled` is true. Active tabs use `tone.accent` + `font-semibold`, inactive `tone.textMuted` + `font-medium`, plus a non-typographic cue: a small `shape.pill` bar (`tone.accentBg`) under the label, transparent when inactive so the layout never shifts. `aria-current` comes from `NavLink` itself.
...
Pure data behind the tab bar, pulled out of `App.tsx` so it can be pinned by a hermetic test without importing the whole app shell: `interface NavTab { to; label; assistant? }`, the `TABS` array (`Capture` `/`, `Today` `/day`, `Recall` `/chat` — `assistant: true`, `Settings` `/settings`), and `visibleTabs(tabs, assistantEnabled)`, a pure filter dropping assistant-gated tabs when disabled. `navTabs.test.ts` pins both the route paths (frozen: `/`, `/day`, `/chat`, `/settings` — independent of any future label change) and the current labels.
...
- `tone` — surface/background (`bg`, `surface`, `sunken`), borders (`border`, `borderStrong`), text ramp (`textPrimary`…`textFaint`, plus `textDerived`), spruce accent (`accent`, `accentBg`, `accentBgActive`, `accentWash`), clay danger (`danger`, `dangerBg`, `dangerBgActive`, `dangerWash`), and `pressWash` for pressed states. Every entry pairs light and `dark:` classes. `textDerived` (#80) is the same hue as `textMuted`, named separately for the authored-vs-generated axis: pairs with `type_.derived` for machine-inferred/generated content (photo captions, related-memory snippets, generated day prose) so the intent reads at each call site rather than overloading `textMuted` for two different reasons.
- `shape` — `card` (14 px radius), `control` (`rounded-xl`), `pill` (`rounded-full`).
- `type_` — typography scale, pinned to a functional contract (#85): serif tokens (`title`, `heading`, `body`, `bodyStrong`, `bodySmall`, `derived`) are for content the user reads as theirs — entry text, day titles, meaningful summaries, machine inference; sans tokens (`sub`, `caption`, `overline`, `ui`) are for chrome — time, duration, state, controls, metadata. `title` (serif 26 px), `heading` (serif 17 px), `body` (serif 16 px), `bodyStrong` (serif 17 px medium, `leading-snug` — main entry text: transcripts and notes, a step above descriptive metadata; the snug leading keeps a two-line-clamped preview reading as a fragment, not a headline), `bodySmall` (serif 14 px — secondary descriptive content like pseudo-entry notes), `derived` (serif 14 px italic, #80 — machine-inferred/generated text: photo captions, related-memory snippets, generated day prose; pairs with `tone.textDerived`; never bolder than `bodyStrong`, pinned by a dedicated `tokens.type.test.ts` case), `sub` (sans 13 px), `caption` (sans 12 px), `overline` (sans 11 px uppercase), `ui` (sans 15 px, "chrome, never serif"). Named `type_` because `type` is reserved-ish. `tokens.type.test.ts` pins the serif/sans partition by asserting every serif key's class string contains `font-serif` (never `font-sans`) and vice versa, over an exhaustive key list — a regression here would silently break the whole discipline.
