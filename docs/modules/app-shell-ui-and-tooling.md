...
- **Background media understanding**: whenever `entries` change, runs `drainTranscriptions(currentStreamId)` and `drainCaptions(currentStreamId)` in parallel; if either produced results, calls `refresh()`, which re-runs the effect until nothing is pending. If a drain completes while the document is hidden, posts a best-effort `showAppNotification` ("N transcripts ready · M captions ready", tag `enrichment-done`) — see [notify.md](notify.md).
